...
class template {
            ...
            if ($num == 1) {
                ...
                if ($nu == 0) {
                    ...
                    mysqli_stmt_bind_param($stmt, "ssssssssssssssssssss",
                        $title, $row['message'], $row['headline'], $row['news'], $row['comment'],
                        $row['usermenu'], $row['usermenu2'], $row['relatedlinks'], $row['commentform'],
                        $row['registerform'], $row['loginform'], $row['logout'], $row['senddataform'],
                        $row['profileform'], $row['archive'], $row['sendnewsform'], $row['addemail'],
                        $row['editemail'], $row['registeremail'], $row['dataemail']);
                    mysqli_stmt_execute($stmt);
    ...
    public function edittemplate(int $templateid, string $delete, string $title, string $message, string $headline, string $news, string $comment, string $usermenu, string $usermenu2, string $relatedlinks, string $commentform, string $registerform, string $loginform, string $logout, string $senddataform, string $profileform, string $archive, string $sendnewsform, string $addemail, string $editemail, string $registeremail, string $dataemail): void {
        global $pn_config, $pn_handler;

        if (!$title || !$message || !$headline || !$news || !$comment || !$usermenu || !$usermenu2 || !$relatedlinks || !$commentform || !$registerform || !$loginform || !$logout || !$senddataform || !$profileform || !$archive || !$sendnewsform || !$addemail || !$editemail || !$registeremail || !$dataemail) {
            ...
            if ($templateid === 1) {
                ...
                if ($delete === "YES") {
                    ...
                    mysqli_stmt_bind_param($stmt, "ssssssssssssssssssssi", $title, $message, $headline, $news, $comment, $usermenu, $usermenu2, $relatedlinks, $commentform, $registerform, $loginform, $logout, $senddataform, $profileform, $archive, $sendnewsform, $addemail, $editemail, $registeremail, $dataemail, $templateid);
                    mysqli_stmt_execute($stmt);
...
class email {
    ...
    public function addemail(string $nickname, string $email, string $password): bool {
        ...
        if ($addemail = $template->addemail($nickname, $email, $password)) {
    ...
    public function editemail(string $nickname, string $email, string $password): bool {
        global $pnconfig;
        $template = new template;
        if ($editemail = $template->editemail($nickname, $email, $password)) {
            $headers = "From: " . L_EMAIL_AUTHOR . " <" . $pnconfig['email'] . ">";
...
class permissions {
    ...
    public function addpermissions(string $user, string $canreadtemplates, string $canwritetemplates, string $canreadconfig, string $canwriteconfig, string $canreadusers, string $canwriteusers, string $canreadpermissions, string $canwritepermissions, string $canreadcategories, string $canwritecategories, string $canreadnews, string $canwritenews, string $canreadcomments, string $canwritecomments): string {
        global $pn_config, $pn_handler;
        $error = '';
        ...
        if ($userid !== false) {
            if (!$this->checkadmin($userid)) {
                ...
                mysqli_stmt_bind_param($stmt, "isssssssssssssss", $userid, $canreadtemplates, $canwritetemplates, $canreadconfig, $canwriteconfig, $canreadusers, $canwriteusers, $canreadpermissions, $canwritepermissions, $canreadcategories, $canwritecategories, $canreadnews, $canwritenews, $canreadcomments, $canwritecomments);
    ...
    public function editpermissions(int $userid, string $canreadtemplates, string $canwritetemplates, string $canreadconfig, string $canwriteconfig, string $canreadusers, string $canwriteusers, string $canreadpermissions, string $canwritepermissions, string $canreadcategories, string $canwritecategories, string $canreadnews, string $canwritenews, string $canreadcomments, string $canwritecomments, string $delete): string {
        global $pn_config, $pn_handler;
        $error = '';
        ...
        if ($this->checkadmin($userid)) {
            if ($delete === "YES") {
                ...
                mysqli_stmt_bind_param($stmt, "ssssssssssssssi", $canreadtemplates, $canwritetemplates, $canreadconfig, $canwriteconfig, $canreadusers, $canwriteusers, $canreadpermissions, $canwritepermissions, $canreadcategories, $canwritecategories, $canreadnews, $canwritenews, $canreadcomments, $canwritecomments, $userid);
                if (!mysqli_stmt_execute($stmt)) {
...
class configuration {
    ...
    public function editconfig(string $categories, string $categorypics, string $comments, string $commentwriting, string $moretext, string $sendnews, string $newssending, string $smilies, string $bbcode, string $html, string $dateformat, string $timeformat, int $template, string $url, string $email, int $headlines, int $news, int $spamprotection, string $relatedlinks, int $relatedlinks_num): string {
        global $pn_config, $pn_handler;
        $error = '';
        ...
        if (preg_match("!^[_a-zA-Z0-9.-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}!", $email)) {
            if (preg_match("!^https?://!", $url)) {
                ...
                mysqli_stmt_bind_param($stmt, "ssssssssssssississsi", $categories, $categorypics, $comments, $commentwriting, $moretext, $sendnews, $newssending, $smilies, $bbcode, $html, $dateformat, $timeformat, $template, $url, $email, $headlines, $news, $spamprotection, $relatedlinks, $relatedlinks_num);
                if (!mysqli_stmt_execute($stmt)) {
            ...
            if ($num2 != 0) {
                ...
                if (preg_match("![./\\\\:*?<>|\"]+!", $name)) {
                    ...
                    if ($pnconfig['categorypics'] == "YES" && $uploadpic === "YES" && !empty($picture['name'])) {
                        $pic = basename((string) $picture['name']);
                        if ($row['picture']) {
                            @unlink('../pngfx/categories/' . $row['picture']);
                        }
    ...
    public function getcomments(int $newsid): void {
        ...
        if ($num == 0) {
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                <?php echo L_NEWS_WRITTENBY; ?> <?php if ($row['userid'] == "0") { echo L_NEWS_GUEST; } else { echo $this->getcommentauthor((int)$row['userid']); } ?> <?php echo L_NEWS_ONDATE; ?> <?php echo date("d.m.Y", (int)$row['time']); ?> <?php echo L_NEWS_AT; ?> <?php echo date("H:i", (int)$row['time']); ?> (IP: <?php echo pnadmin_escape($row['ip']); ?>)
                </td></tr>
                <tr><td valign="top">
