...
class pn_news {
    ...
    public function details(int $newsid): void {
        global $pn_config, $pnconfig, $pn_newsexist, $pn_handler;
    ...
    public function getcatname(int $catid): mixed {
        ...
        if ($num == 1) {
            ...
            $category['description'] = stripslashes(((isset($category['descripiton']) && trim($category['descripiton']) !== '') ? trim($category['descripiton']) : ''));
            return $category;
        }
    ...
    public function commentform(int $newsid): void {
        global $pn_config, $pnconfig, $pnuser;
...
class pn_user {
    ...
    public function checkcookie(): ?array {
        ...
        $cookiestring = base64_decode((string) $_COOKIE['pncookie']);
...
class pn_email {
    ...
    public function registeremail(string $nickname, string $email, string $password): bool {
        global $pnconfig;
        $template = new pn_template;
        if ($registeremail = $template->registeremail($nickname, $email, $password)) {
    ...
    public function dataemail(string $nickname, string $email, string $password): bool {
        global $pnconfig;
        $template = new pn_template;
        if ($dataemail = $template->dataemail($nickname, $email, $password)) {
            $headers = "From: " . L_EMAIL_AUTHOR . " <" . $pnconfig['email'] . ">";
